...
# Copilot Chat metadata — enables using this prompt as a slash command `/k8s-manifest`
name: k8s-manifest
description: Generate production-ready Kubernetes manifests, Helm charts, and GitOps configs tailored to your app.
inputs:
  - name: environment
  ...
  - name: image
    ...
    required: false
  - name: domain
    description: Public domain for ingress (optional)
    required: false
---
...
You can invoke this prompt in Copilot Chat by typing `/k8s-manifest` and supplying optional inputs. Examples:

- `/k8s-manifest app_name=api image=registry/myapp:1.2.3 environment=production domain=app.example.com`
- `/k8s-manifest app_name=frontend environment=staging`

Available inputs: `environment`, `app_name`, `image`, `domain`.
